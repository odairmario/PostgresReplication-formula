...
We will see a quick setup to configure the ``postgresql-replication`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``postgresql-replication`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``postgresql-replication`` formulas under ``/srv/salt/postgresql-replication/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/postgresql-replication/parameters/dns:domain/
    ln -s dns:domain /srv/salt/postgresql-replication/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/postgresql-replication/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/postgresql-replication/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``postgresql-replication`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``postgresql-replication`` formulas under ``/srv/salt/postgresql-replication/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/postgresql-replication/parameters/roles
...
We create a configuration for the role ``postgresql-replication/server`` in ``/srv/salt/postgresql-replication/parameters/roles/postgresql-replication/server.yaml``:
...
We create another configuration for the role ``postgresql-replication/client`` in ``/srv/salt/postgresql-replication/parameters/roles/postgresql-replication/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles postgresql-replication/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles postgresql-replication/client
...
.. code-block:: console
    ...
    mv /srv/salt/postgresql-replication/parameters/dns:domain/  '/srv/salt/postgresql-replication/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    {%- from tplroot ~ "/map.jinja" import mapdata as postgresql-replication with context %}

    test-does-nothing-but-display-postgresql-replication-as-json:
      test.nop:
        - name: {{ postgresql-replication | json }}
...
Here is an example based on `template-formula/postgresql-replication/config/file.sls`_:
...
.. code-block:: sls
    ...
    postgresql-replication-config-file-file-managed:
      file.managed:
        ...
        - mode: 644
        - user: root
        - group: {{ postgresql-replication.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            postgresql-replication: {{ postgresql-replication | json }}

This ``sls`` file expose a ``postgresql-replication`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    {{ postgresql-replication | json }}

    winner of the merge: {{ postgresql-replication['winner'] }}
...
.. _template-formula/postgresql-replication/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/postgresql-replication/config/file.sls
