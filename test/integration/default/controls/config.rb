...
control 'postgresql-replication.config.file' do
  ...
  describe file('/etc/template-formula.conf') do
    ...
    its('content') do
      ...
      should include(
        ...
        '{"postgresql-replication-config-file-file-managed": ["example.tmpl.jinja"], '\
        '"postgresql-replication-subcomponent-config-file-file-managed": '\
